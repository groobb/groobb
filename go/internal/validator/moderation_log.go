...
// ModerationReasonMaxLengthは、モデレーションの操作に記録する理由の最大の長さで、
...
// ModerationLogCreateValidatorは、モデレーションの操作とともに送信された理由を
...
// NewModerationLogCreateValidatorはModerationLogCreateValidatorを生成します。
func NewModerationLogCreateValidator() *ModerationLogCreateValidator {
	return &ModerationLogCreateValidator{}
}

// ModerationLogCreateValidatorInputはModerationLogCreateValidator.Validateの
// 入力です。
type ModerationLogCreateValidatorInput struct {
	Reason string
}

// Validateは送信された理由を検証し、記録される形にしたものを返します。問題があれば、
