...
// create_river_tables migration header: generate clean SQL with `river
// migrate-get --line main --version N --up/--down` (CLI pinned to the go.mod
// River version, version 1 excluded), add it as a new dbmate migration that
// appends `INSERT INTO river_migration (line, version) VALUES ('main', N);`
...
//
// 更新するときは、create_river_tables マイグレーションのヘッダーに記した再現手順に
// 従う: `river migrate-get --line main --version N --up/--down` (go.mod の River
// バージョンに固定した CLI、version 1 は除外) で clean SQL を生成し、末尾に
