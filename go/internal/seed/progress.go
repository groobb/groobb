...
// line with a carriage return, so a step that creates hundreds of rows stays one
// line instead of scrolling the terminal.
//
...
//
// out is still expected to be the stream slog writes to, so that the counter and
// the log lines stay in the order they were written.
//
// [Ja] progress は生成ステップの進み具合を表示します。復帰文字で 1 行を書き換えるため、
// 数百行を作るステップでも表示は 1 行に収まり、ターミナルが流れません。
//
// カウンタを slog ではなく素の io.Writer へ書くのは、これがコマンドを見ている開発者向けの
// 使い捨てのフィードバックであり、実行内容の記録ではないためです。実行内容はステップごとに
// 1 回ログへ残します。
//
// ただし out には slog の書き込み先と同じストリームを渡す前提とします。カウンタとログ行が
// 書いた順に並ぶようにするためです。
...
//
// [Ja] newProgress は label を見出しにして total までを数えるカウンタを開始します。
...
//
// [Ja] advance は項目を 1 件作成したことを記録します。
...
//
// [Ja] finish は行を閉じ、次の出力が行頭から始まるようにします。
...
//
// [Ja] render はカウンタをその場で描き直します。
func (p *progress) render() {
	p.write(fmt.Sprintf("\r  %s %d/%d", p.label, p.current, p.total))
}

// write sends s to the output, discarding any write error on purpose: the output
// carries progress only, and a run that produced the data must not fail because
// the terminal could not be written to.
//
// [Ja] write は s を出力先へ書き、書き込みエラーは意図的に捨てます。出力先が運ぶのは
