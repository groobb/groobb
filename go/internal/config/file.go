...
const (
	// configFileEnvNameは設定ファイルの場所を指す環境変数の名前です。ここで渡された
	...
	// defaultConfigFileNameはconfigFileEnvNameが未設定のときにLoadが作業
	// ディレクトリから読むファイルです。存在しないことはエラーにしません。環境変数だけで
	// 設定するインスタンス (コンテナ・CI・ここの開発環境) も起動できる必要があるためです。
	defaultConfigFileName = "groobb.toml"
)

// fileConfigは設定ファイルのスキーマです。設定1つにつき1フィールドを持ち、
// 運用者が編集する単位でテーブルにまとめています。
//
// テーブルの分け方は環境変数の名前に従い、変数名が示す構成要素をテーブルにします
// (GROOBB_SMTP_HOSTは [email.smtp] の "host"、APP_ENVは [app] の "env")。
// これにより設定ごとの名前が両者で1対1に対応し、一方から他方を読み取れます。
// トップレベルに置く設定はありません。TOMLではテーブルより後ろに書いたキーはその
// テーブルに属するため、トップレベルのキーは手で編集するファイルでは罠になります。
//
// 数値の設定はポインタにして、書かれた値とキーの不在を区別できるようにしています。
// ここにある2つのポートはどちらも0を拒否しますが、0を「不在」に畳み込むと、
...
// loadFileは設定ファイルを読み込みます。既定のファイルが存在しない場合は空の
// fileConfigを返し、すべての設定を環境変数に委ねます。
//
...
// configFilePathは読み込む設定ファイルと、それを運用者自身が指定したものかどうかを
// 返します。
...
// configFileErrorは、設定ファイルの読み込み・デコードの失敗を、問題の位置は示しつつ
...
// formatKeysは設定ファイルのキーをエラーメッセージ向けに整形します。
