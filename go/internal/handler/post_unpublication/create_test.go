...
// TestCreateは、管理者の送信が投稿を視界から外すこと、そのことが伝えられること、そして
...
func TestCreate(t *testing.T) {
	...
	if flash := decodeFlash(t, rec); flash.Type != session.FlashSuccess || flash.Message != "投稿を非公開にしました" {
		t.Errorf("フラッシュ = %+v、期待値は成功の「投稿を非公開にしました」", flash)
	}
	post := findPost(t, f)
	...
	if post.Body != postBody {
		t.Errorf("Body = %q、期待値 = %q", post.Body, postBody)
	}
}
...
// TestCreate_WithoutPermissionは、投稿を視界から外してはならないアカウントが403ページ
...
// TestCreate_MissingPostは、スレッドが発行していない投稿を名指すアドレスが404ページで
// 応答されることを検証します。
...
// TestCreate_UnpublishedThreadは、ページが開かれてから送信が届くまでの間にスレッドが
