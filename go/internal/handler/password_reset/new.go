...
// New GET /password_reset/new - renders the password reset request form with a
// fresh CSRF token.
//
// [Ja] New GET /password_reset/new - 新しい CSRF トークン付きでパスワードリセット申請
// フォームを描画します。
...
// renderNew renders the password reset request form with the given status and
// data. It is shared by New (200) and Create's re-render after a validation
// error (422). The status is written before rendering, so callers pass the final
// status here rather than setting it separately.
//
// [Ja] renderNew は指定したステータスとデータでパスワードリセット申請フォームを描画
// します。New (200) と、Create のバリデーションエラー後の再描画 (422) で共有します。
