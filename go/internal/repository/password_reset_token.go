...
// NewPasswordResetTokenRepository creates a PasswordResetTokenRepository backed
// by the given queries.
//
// [Ja] NewPasswordResetTokenRepository は与えられた queries を使う
...
// 変更しません。
func (r *PasswordResetTokenRepository) WithTx(tx pgx.Tx) *PasswordResetTokenRepository {
...
func (r *PasswordResetTokenRepository) FindByTokenDigest(ctx context.Context, tokenDigest string) (*model.PasswordResetToken, error) {
	row, err := r.q.GetPasswordResetTokenByDigest(ctx, tokenDigest)
	if err != nil {
...
func (r *PasswordResetTokenRepository) MarkAsUsed(ctx context.Context, id model.PasswordResetTokenID) error {
	return r.q.MarkPasswordResetTokenAsUsed(ctx, uuid.UUID(id))
}
...
func (r *PasswordResetTokenRepository) DeleteUnusedByUserID(ctx context.Context, userID model.UserID) error {
	return r.q.DeleteUnusedPasswordResetTokensByUserID(ctx, uuid.UUID(userID))
}

// toModel converts a query.PasswordResetToken row into a
// model.PasswordResetToken, casting the raw uuids into the typed IDs at the
// repository boundary.
//
